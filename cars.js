...
function ferraris(array){
    const ferraris = array.filter((element)=> {
        return element.brand === 'Ferrari';
    })

    return ferraris;
};

function nineties(array){
    const models = array.filter((element)=>{
        return element.model >= 1950 && element.model <= 1980;
    })
...
function list(array){
    const arrayStings = array.map((element)=>{
        return `This ${element.brand} from ${element.model} costs $${element.price}`
    })

    return arrayStings
};

function bmwSum(array){
